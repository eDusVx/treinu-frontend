...
import type { RegisterAlunoPayload, GeneroEnum, ObjetivoEnum } from '../../types'
...
const OBJETIVOS: { value: ObjetivoEnum; label: string }[] = [
...
export default function CadastroAlunoPage() {
  ...
  async function onSubmit(data: FormData) {
    ...
    setLoading(true)
    try {
      const payload: RegisterAlunoPayload = {
      ...
      await alunoService.register(payload)
      navigate('/login', { state: { msg: 'Cadastro realizado! Faça login.' } })
    } catch (err: unknown) {
      ...
      <div className="flex overflow-hidden rounded-[1.3vw] shadow-2xl bg-[#353b37]" style={{ width: '86.7vw' }}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-[2.5vh]">
            <div className="grid grid-cols-2 gap-[2vw]">
              ...
              <div className="flex flex-col gap-[0.7vh]">
                ...
                <select {...register('Objetivo', { required: true })} className={`${inputClass} [color-scheme:dark]`} style={inputStyle}>
                  <option value="">Selecione</option>
                  {OBJETIVOS.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                {errors.Objetivo && <span className="text-red-700 text-xs">Campo obrigatório</span>}
              </div>
        ...
        <div className="flex flex-col justify-center items-center gap-[4vh]" style={{ width: '40%', padding: '4% 6%' }}>
          ...
          <p className="text-white/70 font-medium text-center leading-[1.6]" style={{ fontSize: 'clamp(0.9rem, 1.2vw, 1.3rem)' }}>
            Registre-se como aluno e comece sua jornada com acompanhamento profissional.
          </p>
